...
#### need to not overwrite previous SPF populations when using the function!!! ###

# PigletSelection <- function(piglets, dams, boars) {
  
  #piglets_Fem1 <-   piglets %>% filter(genoA == "a/a" &  sex == "F") %>% top_frac(1, merit)
  
#  piglets_Fem2 <-  piglets %>% filter(genoA %in% c("a/A", "A/a") & sex == "F") %>% top_frac(0.2, merit) 
  
 # if (nrow(filter(dams, genoA %in% c("a/a", "a/A", "A/a")) < 5)) {
  #  piglets_Fem3 <- piglets <- filter(sex == "F" & genoA == "A/A") %>% top_frac(0.1, merit)  }
  ### want to stop even selecting for these pigs after a few generations. ### would be great to know numbers and just use top_n instead then stop selecting A/A when possible
  
  #SPFNucA_Fem <- rbind.fill(piglets_Fem1, piglets_Fem2, piglets_Fem3)
  #piglets_Fem1 <- NULL
  #piglets_Fem2 <- NULL
  #piglets_Fem3 <- NULL
  
  
  ### Selection of Male Piglets by genotype. All resistants kept, some high merit hets
 # piglets_Male1 <-  piglets %>% filter(genoA == "a/a" &  sex == "M") %>% top_frac(1, merit)
  
 # piglets_Male2 <-  piglets %>% filter(genoA %in% c("a/A", "A/a") & sex == "M") %>% top_frac(0.2, merit) 
  
#  if (nrow(filter(boars, genoA %in% c("a/a", "a/A", "A/a")) < 5)) {
 #   piglets_Male3 <- piglets <- filter(sex == "M" & genoA == "A/A") %>% top_frac(0.1, merit)  }
  
  #piglets_Male <- rbind.fill(SPFNucA_Male1, SPFNucA_Male2, SPFNucA_Male3)
  #piglets_Male1 <- NULL
  #piglets_Male2 <- NULL
  #piglets_Male3 <- NULL
...
### bias selection towards resistant or hets in editing population?
  ...
  BreedSPFA_Male <- SPFpop %>% filter (sex == "M" & herd == "A") %>% filter(age >= AgeFirstMate) # %>% top_frac(0.05, merit)
   ...
   #### genotype selection at breeding stage
...
###################
  ...
  if (nrow(filter(BreedSPFB_Male, genoA == "a/a")) >= 3) {
    BreedSPFB_Male <- BreedSPFB_Male %>% filter(genoA == "a/a") # %>% top_frac(0.1, merit) 
  } else if (nrow(filter(BreedSPFB_Male, genoA %in% c("a/a", "a/A", "A/a"))) > 4) {
    BreedSPFB_Male <- BreedSPFB_Male %>% (filter(genoA %in% c("a/a", "a/A", "A/a"))) # %>% top_frac(0.1, merit) 
  ...
  if (nrow(filter(BreedSPFT_Male, genoA == "a/a")) >= 3) {
    BreedSPFT_Male <- BreedSPFT_Male %>% filter(genoA == "a/a") # %>% top_frac(0.1, merit) 
  } else if (nrow(filter(BreedSPFT_Male, genoA %in% c("a/a", "a/A", "A/a"))) > 4) {
    BreedSPFT_Male <- BreedSPFT_Male %>% (filter(genoA %in% c("a/a", "a/A", "A/a"))) # %>% top_frac(0.1, merit) 
