...
edit_genes <- function(popdata, Edit_Efficiency, Embryo_Survival) { 
  ...
  if (nrow(A_ToEdit) > 1){
    ...
    A_ToEditA1 <- A_ToEdit  %>% filter(Allele1 == "A") %>% sample_frac(Edit_Efficiency, replace = FALSE)## need to select at the same time as genes are in the same pig vessel
    ...
    A_ToEditA2 <- A_ToEdit  %>% filter(Allele2 == "A") %>% sample_frac(Edit_Efficiency, replace = FALSE) ## need to select at the same time as genes are in the same pig vessel
    ...
    popdata <- top_frac(popdata, Embryo_Survival, mort) ### selects top 60% of mort column in popdata 
    popdata$mort <- NULL ## removes mortality selection column
    ### add table recording the number of piglets dying - lost productivity potential? 
...
SPFpop <- BasePop(9000, indexSD, AgeDist)  #### creates base population of piggys ### start with ~10,000 to have enough for flowing down ### 
  ...
  for (g in -60:0){
    ...
    #BreedSPFA_Fem <- rbind.fill(BreedSPFA_Fem, SPF_Gilts)
...
if (g >= -35){
 ...
 # write.csv(BurnInPop, paste0("BurnInPop", Sys.Date(), ".csv"))
