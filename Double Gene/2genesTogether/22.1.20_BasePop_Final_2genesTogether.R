...
CreatePiglets <- function (sires, dams, indexSD, genNo, label, littersize){
  ...
  piglets$genoB <- paste(ifelse(sireAlleleA==1,s1B,s2B),ifelse(damAlleleA==1,d1B,d2B), sep = "/")
...
edit_genes <- function(popdata, Edit_Efficiency, Embryo_Survival) { 
  ...
  if (nrow(A_ToEdit) >=1 | nrow(B_ToEdit) >=1) {
    EditedZygotes <- rbind.fill(A_ToEdit, B_ToEdit) %>% distinct() ## creates dataframe of all animals with attempted editing
    
    EditedZygotes$mort <- runif(nrow(EditedZygotes)) ### New column. Random, even 0-1 distribution to all piglets
    EditedZygotes <- top_frac(EditedZygotes, Embryo_Survival, mort) ### selects top 60% of mort column in popdata. Set in function for embryo survival 
    EditedZygotes$mort <- NULL 
    
    popdata <- rbind.fill(EditedZygotes, A_Others, B_Others) ### resistant piglets are remerged with edit attempt piglets
  } 
...
for (g in -60:0){
  ...
  if (sum(ProdPop$age > 8) > 33000){
    ...
    ProdPopFem <- ProdPop %>% filter(sex == "F") %>% filter(genoA != "A/A" & genoB !="B/B") 
    ProdPopFem <- GestationCull(ProdPopFem, 4720)
    
    ProdPopMales <- ProdPop %>% filter (sex == "M") %>% filter(age > AgeFirstMate) %>% filter(genoA != "A/A" & genoB !="B/B") %>% top_n(800, merit)
    ProdPopPiglets <- ProdPop %>% filter(sex == "F") %>% filter(age == 1) %>% filter(genoA != "A/A" & genoB !="B/B") %>% top_n(5000, merit)
  ...
  if (sum(MultPop$age > 8) > 70000) {  ### may be too many!!!
    ...
    MultPopFem <- MultPop %>% filter(sex == "F") %>% filter(genoA != "A/A" & genoB !="B/B") 
    MultPopFem <- GestationCull(MultPopFem, 10000)
    
    MultPopMales <- MultPop %>% filter(sex == "M") %>% filter(age >= AgeFirstMate) %>% filter(genoA != "A/A" & genoB !="B/B")%>% top_n(10000, merit)
    MultPopPiglets <- MultPop %>% filter(sex == "F") %>% filter(age == 1) %>% filter(genoA != "A/A" & genoB !="B/B") %>% top_n(20000, merit) 
    # MultPopPigletsMale <- MultPop %>% filter(sex == "M") %>% filter(age == 1) %>% filter(genoA != "A/A" & genoB !="B/B") %>% top_n(1000, merit) 
...
#write.csv(BurnInPop, paste0("BurnInPop1", Sys.Date(), ".csv"))  
