...
for (g in 0:120){
  ...
  if (nrow(SPFpop %>% filter(sex == "F" & herd == "A") %>% filter(age >= AgeFirstMate & age %% FarrowInt == rem)) >2) {
    ...
    if (nrow(filter(BreedSPFA_Male, genoA == "a/a" & genoB == "b/b")) >= 5) {
      BreedSPFA_Male <- BreedSPFA_Male %>% filter(genoA == "a/a" & genoB == "b/b") # %>% top_frac(0.5, merit) ## Preferbly include but may crash. even 0.9 may be OK selection?
    ...
    } else  (BreedSPFA_Male <- BreedSPFA_Male %>% top_frac(0.05, merit)) ## 5% selection OK??
    ...
    SPFNucA_Fem1 <- newGenSPFNucA %>% filter(genoA == "a/a" & genoB == "b/b" & sex == "F") #%>% top_frac(1, merit) ????
    SPFNucA_Fem <- rbind.fill(SPFNucA_Fem, SPFNucA_Fem1) %>% distinct()
    
    SPFNucA_Male <- filter(newGenSPFNucA, sex == "M") %>% top_frac (0.05, merit)
    SPFNucA_Male1 <-  newGenSPFNucA %>% filter(genoA == "a/a" & genoB == "b/b" &  sex == "M") # %>% top_frac(1, merit)
  ...
  if (nrow(SPFpop %>% filter(sex == "F" & herd == "B") %>% filter(age >= AgeFirstMate & age %% FarrowInt == rem)) >2) {
    ...
    if (nrow(filter(BreedSPFB_Male, genoA == "a/a" & genoB == "b/b")) >= 5) {
      BreedSPFB_Male <- BreedSPFB_Male %>% filter(genoA == "a/a" & genoB == "b/b") # %>% top_frac(0.1, merit)??? 
    ...
    SPFNucB_Fem1 <- newGenSPFNucB %>% filter(genoA == "a/a" & genoB == "b/b" &  sex == "F") # %>% top_frac(1, merit)
    SPFNucB_Fem <- rbind.fill(SPFNucB_Fem, SPFNucB_Fem1) %>% distinct()
    
    SPFNucB_Male <- filter(newGenSPFNucB, sex == "M") %>% top_frac (0.05, merit)
    SPFNucB_Male1 <-  newGenSPFNucB %>% filter(genoA == "a/a" & genoB == "b/b" & sex == "M") #%>% top_frac(1, merit)
  ...
  if (nrow(SPFpop %>% filter(sex == "F" & herd == "T") %>% filter(age >= AgeFirstMate & age %% FarrowInt == rem)) >2) {
    ...
    if (nrow(filter(BreedSPFT_Male, genoA == "a/a" & genoB == "b/b")) >= 3) {
      BreedSPFT_Male <- BreedSPFT_Male %>% filter(genoA == "a/a" & genoB == "b/b")  %>% top_frac(0.5, merit) #### different selection to all the other SPF populations!
    } else if (nrow(filter(BreedSPFT_Male, genoA != "A/A" & genoB != "B/B")) > 4) {
      BreedSPFT_Male <- BreedSPFT_Male %>% filter(genoA != "A/A" & genoB != "B/B")  %>% top_frac(0.5, merit) 
    ...
    SPFNucT_Fem1 <- newGenSPFNucT %>% filter(genoA == "a/a" & genoB == "b/b" &  sex == "F") # %>% top_frac(1, merit)
    SPFNucT_Fem <- rbind.fill(SPFNucT_Fem, SPFNucT_Fem1) %>% distinct()
    
    SPFNucT_Male <- filter(newGenSPFNucT, sex == "M") %>% top_frac (0.1, merit)
    SPFNucT_Male1 <-  newGenSPFNucT %>% filter(genoA == "a/a" & genoB == "b/b" &  sex == "M") #%>% top_frac(1, merit)
  ...
  if (sum(SPFpop$age > 8) > 9000) {
    ...
    SPFpopA_females <- SPFpop %>% filter(sex == "F" & herd == "A") %>% filter(age >= 8) %>% filter (genoA != "A/A" & genoB !="B/B") %>% top_n(4500, merit) 
    SPFpopB_females <- SPFpop %>% filter(sex == "F" & herd == "B") %>% filter(age >= 8) %>% filter (genoA != "A/A" & genoB !="B/B") %>% top_n(2250, merit) 
    SPFpopT_females <- SPFpop %>% filter(sex == "F" & herd == "T") %>% filter(age >= 8) %>% filter (genoA != "A/A" & genoB !="B/B") %>% top_n(2250, merit)
