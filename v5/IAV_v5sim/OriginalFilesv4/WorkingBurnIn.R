...
for (g in -70:0){
  ...
  BreedSPFA_Fem <- filter(SPFpop, sex == "F" & herd == "A") %>% filter(age >= AgeFirstMate & age %% FarrowInt == rem) %>% top_n(100, merit) #crashed when using
  ...
  BreedSPFB_Fem <- filter(SPFpop, sex == "F" & herd == "B") %>% filter(age >= AgeFirstMate & age %% FarrowInt == rem) %>% top_n(100, merit) ### remove these pigs from prod tier. Will leave some pigs that can be bred from!
  
  newGenSPFNucB <- CreatePiglets(BreedSPFB_Male, BreedSPFB_Fem, indexSD, g, paste0("SPFB",g,"_"), littersize)
  
  SPFNucB_Fem <- filter(newGenSPFNucB, sex == "F")  %>% top_frac(0.25, merit) #### maybe need ageDist to make sure pigs of all ages are selected ### 
  ...
  BreedSPFT_Fem <- filter(SPFpop, sex == "F" & herd == "T") %>% filter(age >= AgeFirstMate & age %% FarrowInt == rem ) %>% top_n(100, merit) ### remove these pigs from prod tier. Will leave some pigs that can be bred from!
  
  newGenSPFNucT <- CreatePiglets(BreedSPFT_Male, BreedSPFT_Fem, indexSD, g, paste0("SPFT",g,"_"), littersize)
  
  SPFNucT_Fem <- filter(newGenSPFNucT, sex == "F")  %>% top_frac(0.25, merit) #### maybe need ageDist to make sure pigs of all ages are selected ### 
  ...
  if (g >= -50){ 
    ...
    SPF_ProdPop_Males <- SPFpop %>% filter(sex == "M" & herd == "A") %>% filter(age >= AgeFirstMate) %>% top_frac(0.05, merit) 
    
    ProdPop <- rbind.fill(ProdPop, SPF_ProdPop_Fem) ### No males from SPF stored as used by AI only . Only PN bred males will be in PN ######
    
    Prod_Males <- rbind.fill(ProdPop, SPF_ProdPop_Males) %>% filter(sex == "M") %>% filter(age >= AgeFirstMate) %>% top_frac(0.1, merit) # selects for males from ProdPop and SPF Nucleus 
    
    Prod_Females <- ProdPop %>% filter(sex == "F") %>% filter(age >= AgeFirstMate & age %% FarrowInt == rem) %>% top_n(500, merit) %>% mutate(ID = as.character(ID))### already over age first mate, ensure they are correct farrowing interval. 
  ...
  if (g >= -35){
    ...
    SPF_MultPop_Males <- SPFpop %>% filter(sex == "M" & herd == "B") %>% filter(age >= AgeFirstMate) %>% top_n(100, merit) 
  ...
  if (g >= -25){
    ...
    NewBW_Females <- BW_piglets %>% filter(sex == "F") %>% top_frac(0.3, merit) 
  ...
  if (sum(BWpop$age > 8) > 200000) {  
    ...
    BWpopMales <- BWpop %>% filter(sex == "M") %>% filter(age >= AgeFirstMate)  %>% top_n(2000, merit)
